...
def evaluate(model, data_iterator, params, mark='Eval', verbose=False):
    ...
    batch_output = model(batch_data, token_type_ids=None, attention_mask=batch_masks, labels=batch_tags)  # shape: (batch_size, max_len, num_labels)
...
if __name__ == '__main__':
    ...
    with open(os.path.join(args.data_dir, 'test', 'sentences.txt'), 'r') as fp:
        batch_data = fp.read()
        fp.close()
